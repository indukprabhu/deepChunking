...
import polyglot.visit.ChunkingForLoop;
import polyglot.visit.LoopChunk;
...
public class Optimizer {
    ...
    private List<Goal> preInlinerGoals() {
        ...
        if (config.LOOP_OPTIMIZATIONS) {
            ...
            /**
             * @Indu
             * adding Loop Chunking goal
             * 
             */
           // goals.add(LoopChunking());
     ...
     * @Indu
     * 
     * adding Loop Chunking function.
     *  
     * 
     * @return goal of LoopChunk visitor
     */
    public Goal LoopChunking() {
        NodeVisitor visitor = new LoopChunk(job, ts, nf);
        Goal goal = new ValidatingVisitorGoal("LoopChunking", job, visitor);
        return goal.intern(scheduler);
    }

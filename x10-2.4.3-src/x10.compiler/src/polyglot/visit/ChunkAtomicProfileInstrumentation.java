...
public class ChunkAtomicProfileInstrumentation extends ContextVisitor {
	...
	public ChunkAtomicProfileInstrumentation(Job job, TypeSystem ts, NodeFactory xnf) {
		super(job, ts, xnf);
		this.ts = ts;
		this.xnf = xnf;
	...
	protected Node leaveCall(Node parent, Node old, Node n, NodeVisitor v)
		...
		*/
		if (n instanceof For_c) {
			...
			// Assumes that a par-for-loop requires a Finish enclosing it and a
			...
			if (enclFinish != null && asyncChild != null) {
			   ...
			   for(MethodDecl mi : funToVisit){
				...
				//asyncChild.prettyPrint(System.out);
				
				int id = StatementAtomicProfiler.id;
			...
			// change the for.
				...
				if(forbody instanceof Async_c){
					forNode = (For_c)forNode.body(asyncChild.body());
				}
...
class QueryUserFunctionCalls extends ContextVisitor {
	...
	private Synthesizer synth;
	private Job job;
	public static List<MethodDecl> funToVisit;
	private static Position PCG = Position.COMPILER_GENERATED;

	

	public QueryUserFunctionCalls(Job job, TypeSystem ts, NodeFactory xnf) {
		super(job, ts, xnf);
		this.job = job;
		this.ts = ts;
		this.xnf = xnf;
		funToVisit = new ArrayList<MethodDecl>();
		syn = new AltSynthesizer(ts, xnf);
		synth = new Synthesizer(xnf, ts);
	...
	protected Node leaveCall(Node parent, Node old, Node n, NodeVisitor v)
		...
		if(n instanceof X10Call_c){
			List<functionTableElement> tempListFuncs = new ArrayList<functionTableElement>();
			tempListFuncs.addAll(FunctionTable.funTable);
			for(functionTableElement fi : FunctionTable.funTable){
			   if( ((X10Call_c)n).name().toString().equals(fi.funName) ){
				   boolean present =false;
				   MethodDecl mi = fi.funNode;
				   for(MethodDecl m : funToVisit){
					   if((mi.name().toString().equals(m.name().toString())) ){
						   present =true;
						 break;
					   }
				   }

 ...
 * 
 */
public class CostFunctionGenerator extends ContextVisitor {
	...
	public CostFunctionGenerator(Job job, TypeSystem ts, NodeFactory xnf,Node parentAsync) {
		super(job, ts, xnf);
		this.ts = ts;
		this.xnf = xnf;
			...
			for (Stmt s : bodyBlock) {
				...
				//if it is a statement with a user defined func call then also not do profiling.
				 ...
				 //if it is user defined func call
					 ...
					 //check the cost function after replcing, if it is being redefined before or not.
					 // if a conditional statements condition is redefined then the max of then cost and else cost is taken.
					 //for each such change an id for identification and updation is kept.and checked here.
					 // if a redefinition is happended for a multiply statement ie in left or right arguments then 
					 // a flag is set to indicate a bound redefinition.
					 checkVarRedifinitionAfterFunCall checVar = new checkVarRedifinitionAfterFunCall(job, ts, xnf, parentAsync, s);
					 funcCostFunc = checVar.begin().visitEdgeNoOverride(null, funcCostFunc);
					 
					 if(checVar.isBoundRedefined==false && (checVar.ifRedefined == checVar.ifRedefinitionFixed)){
						 costFunction = syn.createBinary(PCG, costFunction, Binary.ADD, ((Eval_c)funcCostFunc).expr(), this);
					 }
					 else{
					//System.out.println(" the variables are redefined Error Cannnot formulate loop");
					 isBoundRedefined = true;
					 costFunction = syn.createBinary(PCG, costFunction, Binary.ADD, syn.createLongLit(1), this);
					 }
...
class checkVarRedifinitionAfterFunCall extends ContextVisitor {
	...
	private Synthesizer synth;
	private Node parentAsync;
	private Node curStmt;
	public static boolean isBoundRedefined;
	public static int ifRedefined;
	public static int ifRedefinitionFixed;
	public static Expr boundRedifinedExpr;
	public static Expr costToBeAddedLater;
	

	private static Position PCG = Position.COMPILER_GENERATED;

	public checkVarRedifinitionAfterFunCall(Job job, TypeSystem ts, NodeFactory xnf,Node parentAsync,Node curStmt) {
		super(job, ts, xnf);
		this.ts = ts;
		this.xnf = xnf;
		this.parentAsync=parentAsync;
		...
		isBoundRedefined = false;
		ifRedefined = 0;
		ifRedefinitionFixed = 0;
		boundRedifinedExpr = null;
		costToBeAddedLater = null;
	}
	/*
	 * @author Indu
	 * 
	...
	protected Node leaveCall(Node parent, Node old, Node n, NodeVisitor v)
			throws SemanticException {
		if (n instanceof Conditional_c && !isBoundRedefined) {
			
			Node costFunction = n;
			Conditional_c condStmt = ((Conditional_c) n);
			Expr cond = condStmt.cond();

			
			
			Expr thenCost = (condStmt).consequent();

			
			Expr elseCost = condStmt.alternative();
			
			/*
			 * check the def-use of the condition 1)get the uses of the
			 * variables in the expression specified. 2)check wether thr
			 * variable is redefined anywhere before this use.
			 */
			getVarUse getuses_v = new getVarUse(job, ts, xnf);
			getuses_v.begin().visitEdge(null, cond);
			List<Name> listOfUses = getVarUse.listUses;

			//System.out.println("After procedure call : variables in If ");
			for (Name d : listOfUses) {
				//System.out.println(d);
			}
			boolean isRedefined = false;
			// //System.out.println(" position "+((Stmt)n).position() +
			// " "+((Stmt)n).firstChild().position());
			for (Name ld : listOfUses) {
				DefUseCheckerChunking defUsecheck_v = new DefUseCheckerChunking(
						job, ts, xnf, ld, (Stmt)curStmt);
				...
				/*
				 * if(DefUseCheckerChunking.redefinedVarToReplace!=null){
				 * replaceOldVarToNewVar replace_v = new
				 * replaceOldVarToNewVar(job, ts, xnf,
				 * DefUseCheckerChunking.redefinedVarToBeReplaced,
				 * DefUseCheckerChunking.redefinedVarToReplace);
				 * cond=(Expr)replace_v.begin().visitEdgeNoOverride(null,cond);
				 * cond.prettyPrint(System.out); }
				 */
			...
			if (isRedefined) {
				//System.out.println("Yes it is redefined.");
				// make copy of then and else cost as we are using the same
				// expression twice in the cost function now.
				DuplicateSubtreeAndRenameLocals dup_v = new DuplicateSubtreeAndRenameLocals(
						job, ts, xnf);
				Expr thenCostCopy = (Expr) dup_v.begin().visitEdgeNoOverride(
						null, thenCost);

				DuplicateSubtreeAndRenameLocals dup_v2 = new DuplicateSubtreeAndRenameLocals(
						job, ts, xnf);
				Expr elseCostCopy = (Expr) dup_v2.begin().visitEdgeNoOverride(
						null, elseCost);

				Expr maxCost = syn.createBinary(PCG, thenCost, Binary.GE,
						elseCost, this);
				costFunction = xnf.Conditional(PCG, maxCost, thenCostCopy,
						elseCostCopy).type(thenCostCopy.type());
			} 
		...
		if (n instanceof X10Call_c && ((X10Call_c)n).name().toString().equals("operator*") && !isBoundRedefined) {
			
			Expr right = ((X10Call_c)n).arguments().get(0);
			Expr left = (Expr) ((X10Call_c)n).target();
			
			/*
			 * check the def-use of the lefy 1)get the uses of the
			 * variables in the expression specified. 2)check wether thr
			 * variable is redefined anywhere before this use.
			 */
			getVarUse getuses_v = new getVarUse(job, ts, xnf);
			getVarUse.listUses.clear();
			getuses_v.begin().visitEdge(null, left);
			List<Name> listOfUses = getVarUse.listUses;
			
			
			
			//System.out.println("After procedure call : variables in loop left ");
			for (Name d : listOfUses) {
				//System.out.println(d);
			}
			boolean isRedefined = false;
			//check the list of uses
			for (Name ld : listOfUses) {
				DefUseCheckerChunking defUsecheck_v = new DefUseCheckerChunking(
						job, ts, xnf, ld,(Stmt)curStmt);
				DefUseCheckerChunking.isRedefined = false;
			...
			if(isRedefined ){
				//System.out.println("loop bound redefined");
				isBoundRedefined = true;
				boundRedifinedExpr = left;
				costToBeAddedLater = right;
			}
			isRedefined= false;
			...
			getVarUse.listUses.clear();
			getuses_v2.begin().visitEdge(null, right);
			List<Name> listOfUses2 = getVarUse.listUses;
			for (Name ld : listOfUses2) {
				DefUseCheckerChunking defUsecheck_v = new DefUseCheckerChunking(
						job, ts, xnf, ld,(Stmt)curStmt);
				DefUseCheckerChunking.isRedefined = false;
				DefUseCheckerChunking.redefinedVarToReplace = null;
			...
			if(isRedefined ){
				//System.out.println("loop bound redefined");
				isBoundRedefined = true;
				boundRedifinedExpr = right;
				costToBeAddedLater = left;
			}

...
public class CustomChunk extends ContextVisitor {
	...
	public static boolean isConstantBound = false;
	
	public CustomChunk(Job job, TypeSystem ts, NodeFactory xnf) {
		super(job, ts, xnf);
		this.job = job;
		this.ts = ts;
		this.xnf = xnf;
	...
	protected Node leaveCall(Node parent, Node old, Node n, NodeVisitor v)
		...
		/*
		 * class changed to include/exclude global chunking variables.
		 */
		if (n instanceof polyglot.ast.ClassDecl) {
			
			if(enclosingClass!=null && ((ClassDecl)n).name().toString().equals(enclosingClass.name().toString())){
				...
				if(mi instanceof X10FieldDecl_c && (((X10FieldDecl_c) mi).name().toString().contains("isPopulated") ||((X10FieldDecl_c) mi).name().toString().contains("myChunks")) ){
					for(String si :listNameToBeKept){
						if((((X10FieldDecl_c) mi).name().toString().equals(si))){
							listNewMem.add(mi);
							listNewField.add(((X10FieldDecl)mi).fieldDef());
						}
					}
				}
				else{
		...
		if (n instanceof For_c) {
			...
			if (enclFinish != null && asyncChild != null) {
				...
				else if(config.CHUNK_DYNAMIC){
				  ////System.out.println("Dynamic Chunking found");
				  return performDynamicChunk((For_c)n, asyncChild);
					
				}
				else if(config.CHUNK_GUIDED){
					 ////System.out.println("Guided Chunking found");	
					 return performGuidedChunk((For_c)n, asyncChild);
				}
				else if(config.CHUNK_HISTORY){
					 ////System.out.println("HISTORY Chunking found");	
					...
					if(enclosingClass==null || !(tempClass.name().toString().equals(enclosingClass.name().toString()))){
					 ...
					 return performHistoryChunk((For_c)n, asyncChild, finishId);
				}
				
				else if(config.CHUNK_CUSTOM ){
				// get the enclosing class decl of this par-for-loop.
				// requires the traversal from root than traversing backwards.
				if(config.FINISH_ID==finishId){
					////System.out.println("NOT chunking finish "+finishId);
					return performBlockChunk((For_c)n, asyncChild);
				}
				/*if(config.FINISH_ID != finishId){
						return n;
				...
				transformParFor trans_v = new transformParFor(job, ts, xnf);
				trans_v.begin();
...
*/
	 ...
	 * return null, but changes the class variable finalNode corresponding to transformed for block
	 */
	public Stmt performDynamicChunk(For_c parForNode,Async_c asyncNode)  throws SemanticException{
		Stmt forNode = parForNode;
		Stmt forBody = parForNode.body();
		//nproc decl.
		LocalDecl nprocDec = addLocals("nproc");
		...
		//new DyncSched object.
		getDynSched getDyncSched_v = new getDynSched(job, ts, xnf);
		getDyncSched_v.begin();
		job.ast().visit(getDyncSched_v);
		ClassDecl_c dyncSchedClass=getDyncSched_v.workDividerClass;
		
		Expr newSched = synth.makeNewInstance(PCG,
				dyncSchedClass.classDef(), new ArrayList<Type>(),
					new ArrayList<Expr>(), null, context);
		//sched variable
		LocalDecl schedDecl = syn.createLocalDecl(PCG, Flags.FINAL,Name.make("sched"),dyncSchedClass.classDef().asType(),newSched );
		Local schedLocal = syn.createLocal(PCG,schedDecl);
		...
		//mychunk = sched.divideWork(nproc,tproc,n)
				List<Expr> divideCallArgs = new ArrayList<Expr>();
				divideCallArgs.add(nprocLocal);
				divideCallArgs.add(tprocLocal);
				divideCallArgs.add(nLocal);
				
				List<Type> divideCallArgType = new ArrayList<Type>();
				divideCallArgType.add(ts.Long());
				divideCallArgType.add(ts.Long());
				divideCallArgType.add(ts.Long());
				
				Call divideWorkCall = synth.makeInstanceCall(PCG, schedLocal, Name.make("scheduleDynamic"),
						new ArrayList<TypeNode>(), divideCallArgs, chunkClassDecl.classDef().asType(), 
						divideCallArgType, context);
				
				
				LocalDecl mychunkDecl = syn.createLocalDecl(PCG, Flags.MUTABLE, Name.make("myChunk"), 
						chunkClassDecl.classDef().asType(), divideWorkCall);
				Local mychunkLocal = syn.createLocal(PCG, mychunkDecl);
				Expr myChunkAssignExpr = syn.createAssign(PCG, mychunkLocal, Operator.ASSIGN, divideWorkCall, this);
				
//				Stmt myChunkAssignment = syn.createAssignment(PCG, mychunkLocal,
//						Operator.ASSIGN, divideWorkCall, this);
//			
	  ...
	  for(Stmt s : ((Block_c)oldAsyncBody).statements()){
		...
		//create variables for inner block chunked for loop
			...
			LocalDecl kkmaxDecl = syn.createLocalDecl(PCG,Flags.FINAL, Name.make("kkmax"),ts.Long(),
					syn.createFieldRef(PCG, mychunkLocal,Name.make("myChunkEnd")));
		  ...
		  LocalDecl kkDecl = syn.createLocalDecl(PCG,Flags.MUTABLE,Name.make("kk"),ts.Long(),
				  syn.createFieldRef(PCG, mychunkLocal,Name.make("myChunkStart")));
		  Local kkLocal = syn.createLocal(PCG,kkDecl);
		  Expr kkLTkkmax = syn.createBinary(PCG, kkLocal,Binary.LE, kkmaxLocal, this);
		  Expr kkkInc = syn.createAssign(PCG,kkLocal,Operator.ADD_ASSIGN,syn.createIntLit(1),this);
         ...
         Block newAsyncBody = syn.createBlock(PCG, kkToIAsyncList);
			...
			Stmt innerFor = syn.createStandardFor(PCG,kkDecl,kkLTkkmax, kkkInc,newAsyncBody);                
			

			Expr myChunkEmptyCond = syn.createBinary(PCG, syn.createFieldRef(PCG, mychunkLocal,Name.make("myChunkStart")),
					Binary.GE, syn.createLongLit(0), this);
			List<Stmt> midForStmts = new ArrayList<Stmt>();
			midForStmts.add(kkmaxDecl);
			midForStmts.add(innerFor);
			Block midForBody = syn.createBlock(PCG,midForStmts);
			Stmt midFor = syn.createStandardFor(PCG, mychunkDecl,myChunkEmptyCond ,myChunkAssignExpr , midForBody);
			
			
			List<Stmt> newAsyncStmt = new ArrayList<Stmt>();
			newAsyncStmt.add(midFor);
	 ...
	 * return null, but changes the class variable finalNode corresponding to transformed for block
	 */
	public Stmt performGuidedChunk(For_c parForNode,Async_c asyncNode)  throws SemanticException{
		...
		//new GuidcSched object.
		getGuidSched getGuidcSched_v = new getGuidSched(job, ts, xnf);
		getGuidcSched_v.begin();
		job.ast().visit(getGuidcSched_v);
		ClassDecl_c GuidcSchedClass=getGuidcSched_v.workDividerClass;
		
		Expr newSched = synth.makeNewInstance(PCG,
				GuidcSchedClass.classDef(), new ArrayList<Type>(),
					new ArrayList<Expr>(), null, context);
		//sched variable
		LocalDecl schedDecl = syn.createLocalDecl(PCG, Flags.FINAL,Name.make("sched"),GuidcSchedClass.classDef().asType(),newSched );
		Local schedLocal = syn.createLocal(PCG,schedDecl);
		...
		//mychunk = sched.divideWork(nproc,tproc,n)
				...
				Call divideWorkCall = synth.makeInstanceCall(PCG, schedLocal, Name.make("scheduleGuided"),
						new ArrayList<TypeNode>(), divideCallArgs, chunkClassDecl.classDef().asType(), 
						divideCallArgType, context);
...
//			
	 ...
	 * return null, but changes the class variable finalNode corresponding to transformed for block
	 */
	public Stmt performHistoryChunk(For_c parForNode,Async_c asyncNode, int forId)  throws SemanticException{
		Stmt forNode = parForNode;
		Stmt forBody = parForNode.body();
		//nproc decl.
		LocalDecl nprocDec = addLocals("nproc");
		...
		getHistSched getHistSched_v = new getHistSched(job, ts, xnf);
		getHistSched_v.begin();
		getHistSched.workDividerClass = null;
		job.ast().visit(getHistSched_v);
		//System.out.println("found "+getHistSched_v.workDividerClass.toString());
		ClassDecl_c histSchedClass=getHistSched.workDividerClass;
		
		Expr newSched = synth.makeNewInstance(PCG,
				histSchedClass.classDef(), new ArrayList<Type>(),
					new ArrayList<Expr>(), null, context);
		//sched variable
		LocalDecl schedDecl = syn.createLocalDecl(PCG, Flags.FINAL,Name.make("sched"),histSchedClass.classDef().asType(),newSched );
		Local schedLocal = syn.createLocal(PCG,schedDecl);
		...
		Expr newChunk = synth.makeNewInstance(PCG,
				...
				// get the histProfileObj  node.
		getHistProfileObj histProfileObj_v = new getHistProfileObj(
						job, ts, xnf);
		histProfileObj_v.begin().visitEdgeNoOverride(null, job.ast());
		X10FieldAssign_c histProfileObj = getHistProfileObj.obj;
		...
		List args = new ArrayList<Expr>();
		args.add(syn.createLongLit(finishId));
		args.add(nLocal);
		args.add(costArrayLocal);
		
		List argsType = new ArrayList<Type>();
		argsType.add(ts.Long());
		argsType.add(ts.Long());
		argsType.add(costArrayLocal.type());
			
			
			
			Expr histObjLocal = synth.makeFieldAccess(PCG, syn.createThis(PCG,enclosingClass.classDef().asType()), Name.make("histObj") , context);
			
		Expr costArrayExpr =synth.makeInstanceCall(PCG, histObjLocal, Name.make("populateArray"),
				new ArrayList<TypeNode>() ,args, ts.Long(),argsType ,context);
		Stmt costArrayReAssign = syn.createAssignment(PCG, costArrayLocal,Operator.ASSIGN,costArrayExpr , this);		
		...
		//idle threads to be passed
		Expr idlethreadAccess = synth.makeStaticCall(PCG,ts.Runtime(), Name.make("retIdleWorkers"), new ArrayList<TypeNode>(),
				new ArrayList<Expr>(), ts.Long(), new ArrayList<Type>(), context);
		...
		//mychunk = sched.divideWork(nproc,tproc,n)
				...
				divideCallArgs.add(tprocLocal);
				divideCallArgs.add(nLocal);
				divideCallArgs.add(costArrayLocal);
				divideCallArgs.add(idlethreadAccess);
				...
				divideCallArgType.add(ts.Long());
				divideCallArgType.add(ts.Long());
				divideCallArgType.add(costArrayLocal.type());
				divideCallArgType.add(ts.Long());
				Call divideWorkCall = synth.makeInstanceCall(PCG, schedLocal, Name.make("scheduleHistory"),
						new ArrayList<TypeNode>(), divideCallArgs, chunkClassDecl.classDef().asType(), 
						divideCallArgType, context);

...
public class StatementAtomicProfiler extends ContextVisitor {
	...
	public StatementAtomicProfiler(Job job, TypeSystem ts, NodeFactory xnf) {
		super(job, ts, xnf);
		this.ts = ts;
		this.xnf = xnf;
		...
		if(n instanceof Block_c){
			...
			for(Stmt s:blockStmts){
			 ...
			 if(basicBlockStatus == 0){
				 ...
				 //if block not open then start a new block when you see a statement other than if / for other constructs.
				 ...
				 //if it is a if or for, then simply add
				 else{
						newBlockStmt.add(s);
					}
				}
						...
						id=id+1;
						 ...
						 //else when not a special construct check if it is atomic
						 
						 else if( s instanceof Atomic_c){
						//if the statement is an atomic block then finish the previous block,
						//add the current block in a new startTimer and finish the atomic block
						{
						List<Type> argTypesTimer= new ArrayList<Type>();
						Expr argsTimer[] = new Expr[0];
						Expr timerCall =syn.createStaticCall(PCG,(polyglot.types.Type)ts.System(),Name.make("nanoTime"), argsTimer);
						...
						id =id+1;
						//startTimer for atomic block
						Expr startTimeAssignCallAtomic = makeArrayAssign(startTimer,ts.Long()
					    		,syn.createLongLit(id), ts.Long(),timerCall, context);
						Stmt startTimerAssignStmtAtomic = syn.createEval(startTimeAssignCallAtomic);
						
						newBlockStmt.add(startTimerAssignStmtAtomic);
						newBlockStmt.add(s);
						}
						//end Timer for the atomic
						{
							List<Type> argTypesTimer= new ArrayList<Type>();
							Expr argsTimer[] = new Expr[0];
							Expr timerCall =syn.createStaticCall(PCG,(polyglot.types.Type)ts.System(),Name.make("nanoTime"), argsTimer);
						...
						id=id+1;
						 ...
						 //otherwise simply add the statement
						 else{
								newBlockStmt.add(s);
							}
					}					

...
* 
*/
class AtomicCountFuncGenerator extends ContextVisitor {
	...
	public AtomicCountFuncGenerator(Job job, TypeSystem ts, NodeFactory xnf,Node parentAsync) {
		super(job, ts, xnf);
		this.ts = ts;
		this.xnf = xnf;
  ...
  public Node override(Node n) {
	...
	protected Node leaveCall(Node parent, Node old, Node n, NodeVisitor v)
		...
		if (n instanceof Loop_c) { // retrun a stmt equals to its cost
			...
			Expr costLoopHeader = syn.createLongLit(0);//for time being adding it as 200 ms
			...
			if(isRedefined){
				...
				//costFunction = syn.createLongLit(0);
				 ...
				 Stmt prev = null ;
						 if ( bodyBlock.indexOf(s) > 0 ) { 
							 ...
							 if(( prev instanceof Eval_c) &&  ((Eval_c)prev).expr() instanceof X10Call_c  && ((X10Call_c)((Eval_c)prev).expr()).name().toString().startsWith("operator()=") 
								 ...
								 Expr myCost = syn.createLongLit(1);

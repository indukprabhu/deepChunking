...
public abstract class Scheduler {
    ...
    /** @Suyash -- To call the goal CountPassInfo and CountAsyncs
     * passOn=0 -- CountPassInfo
     * passOn=1 -- CountAsyncs
     * passOn=2 -- ModifyAsyncs
     */
    public static int passOn	= 0;		
    public static int fstable	= 0;
    public static int dAnalysis = 0;
...
//            ts.restoreSystemResolver(s.resolver);
    ...
    protected boolean runPass(Goal goal) throws CyclicDependencyException {
        ...
        if (true || job == null || job.status()) {
            ...
            try {
            	// @Suyash
            	/*if(goal.name().equals("CountPassInfo")) {
            		System.out.println("entered goal: CountPassInfo: " + passOn);
            	}
            	if(goal.name().equals("ModifyAsyncs"))
            		passOn++;
            	*/
            	/*if(goal.name().equals("LocalResetter")) {
            		System.out.println("entered goal: RenamingData: " + rename);
            	}
            	if(goal.name().equals("ReduceAsyncs")) {
            		System.out.println("entered goal: ReduceAsyncs: " + passOn);
            		
            	}*/
            	/*if(goal.name().equals("CodeGenerated")) {
            		System.out.println("entered goal: CodeGenAST: ");
            	}*/
            	
            	
            	if(goal.name().equals("DepAnalyzerP0"))
            		System.out.println("entered goal: Dependence Analysis Phase 0: " + dAnalysis);
            	else if(goal.name().equals("DepAnalyzerP1"))
            		dAnalysis++;
            	else if(goal.name().equals("DepAnalyzerP2"))
            		dAnalysis++;
            	else if(goal.name().equals("DepAnalyzerP3"))
            		dAnalysis++;
            	else if(goal.name().equals("DepAnalyzerP4"))
            		dAnalysis++;
            	
            	/*if(goal.name().equals("FinStablizer"))
            		System.out.println("entered goal: Finish Stabalizer: " + fstable);
            	*/
                result = goal.runTask();
                
                if(goal.name().equals("DepAnalyzerP4"))
                	dAnalysis++;
                /*if(goal.name().equals("FinStablizer"))
                	fstable=2;
                */
